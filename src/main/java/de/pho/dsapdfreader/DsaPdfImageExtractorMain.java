...
public class DsaPdfImageExtractorMain extends PDFStreamEngine {
        ...
        configs.stream()
                ...
                .forEach(conf -> {
                    ...
                    try {
                        ...
                        document = loadDocument(path, conf.pdfName); //PDDocument.load(new File(PDF_BASE_PATH + path + "/" + conf.pdfName));
    ...
    private static PDDocument loadFromPath(String fullPath) {
        PDDocument document = null;
        try {
            document = PDDocument.load(new File(fullPath));
        } catch (IOException e) {
            System.err.println("Failed to load document from path: " + fullPath);
        }
        return document;
    }
