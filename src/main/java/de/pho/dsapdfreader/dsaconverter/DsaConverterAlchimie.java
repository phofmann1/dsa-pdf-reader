...
public class DsaConverterAlchimie extends DsaConverter<AlchimieRaw, ConverterAtomicFlagsAlchimie> {
    ...
    protected void applyFlagsForKey(String key) {
        ...
        this.getFlags().wasWertPreis.set(key.equals(KEY_WERT_PREIS));
        this.getFlags().wasTypicalIngredients.set(key.equals(KEY_TYPISCHE_INGREDIENZIEN));
        this.getFlags().wasCost.set(key.equals(KEY_KOSTEN));
        this.getFlags().wasLabor.set(key.equals(KEY_LABOR));
        this.getFlags().wasBrewingDifficulty.set(key.equals(KEY_BRAUSCHWIERIGKEIT));
        this.getFlags().wasRequirements.set(key.equals(KEY_VORAUSSETZUNGEN));
        this.getFlags().wasApValue.set(key.equals(KEY_AP_WERT));
        this.getFlags().wasQualityLevels.set(key.equals(KEY_QUALITAETSSTUFEN) || key.equals(KEY_QUALITAETSSTUFEN_2));
        this.getFlags().wasAlternativeNamen.set(key.equals(KEY_ALTERNATIVE_NAMEN) || key.equals(KEY_ALTERNATIVE_NAMEN_2) || key.equals(KEY_ALTERNATIVE_NAMEN_3));
    ...
    protected void applyDataValue(AlchimieRaw currentDataObject,
        ...
        if (this.getFlags().wasRequirements.get())
            currentDataObject.requirements =
                    concatForDataValueWithMarkup(currentDataObject.requirements, cleanText, isBold, isItalic);
